...
describe('sendextra errorhandling', function () {
  var msg_small =
  {
  ...
  it('should warn  if there is a problem with json paths and senderr is set to false', function (done) {
    helper.load(requiredNodes, flow, function () {
      ...
      sendextra_n.text = "payload.my_faulty_text";

      var helper_n = helper.getNode("helper");
      sendextra_n.senderr = false;
      sendextra_n.emit("input", msg_big );
    ...
    helper.load(requiredNodes, flow, function () {
      ...
      helper_n.on("input", (msg) => {
        try {
          ...
          expect(msg.responses).to.be.like(msg_big.responses);
          done();
        } catch (e) {
          done(e);
        }
